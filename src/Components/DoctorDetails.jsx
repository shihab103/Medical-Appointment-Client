    ...
    <div className="px-20"> 
      ...
      <div className="flex bg-white p-7 rounded-2xl">
        ...
        <div className="content-center">
          ...
          <h1 className="font-semibold mb-2">{singleDoctor.workingAt}</h1>
          <hr className="border-dashed border-gray-400" />

          <p className="text-gray-500 my-2">
            Reg NO: {singleDoctor.registrationNumber}
          </p>
          <hr className="border-dashed border-gray-400" />
          <div className="flex items-center">
            <span className="font-bold">Availability: </span>
            {singleDoctor.availability.map((day, id) => (
              <h1 key={id} className="mt-2 mr-2 bg-[#fff5e5] px-3 py-1 text-yellow-500 rounded-full">
                {day}
              </h1>
            ))}
          </div>
          <h2>
            <span className="font-bold">consultation Fee:</span> Taka:{" "}
